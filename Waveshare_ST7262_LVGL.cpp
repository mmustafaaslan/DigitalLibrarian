...
IRAM_ATTR static void rotate_copy_pixel(const lv_color_t *from, lv_color_t *to,
...
static void flush_dirty_copy(void *dst, void *src,
  ...
  for (int i = 0; i < dirty_area->inv_p; i++) {
    ...
    if (dirty_area->inv_area_joined[i] == 0) {
      ...
      rotate_copy_pixel((lv_color_t *)src, (lv_color_t *)dst, x_start, y_start,
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  if (lv_disp_flush_is_last(drv)) {
    ...
    if (drv->full_refresh) {
      ...
      // Roate and copy data from the whole screen LVGL's buffer to the next
      // frame buffer
      next_fb = flush_get_next_buf(lcd);
      rotate_copy_pixel((lv_color_t *)color_map, (lv_color_t *)next_fb,
                        offsetx1, offsety1, offsetx2, offsety2, LV_HOR_RES,
      ...
      lcd->drawBitmap(offsetx1, offsety1, offsetx2 - offsetx1 + 1,
                      offsety2 - offsety1 + 1, (const uint8_t *)next_fb);
...
void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area,
   ...
   * frame buffer */
  rotate_copy_pixel((lv_color_t *)color_map, (lv_color_t *)next_fb, offsetx1,
...
static void update_callback(lv_disp_drv_t *drv) {
  ...
  ESP_LOGD(TAG, "Current mirror x: %d, mirror y: %d, swap xy: %d",
           lcd->getMirrorXFlag(), lcd->getMirrorYFlag(), lcd->getSwapXYFlag());
}
...
void rounder_callback(lv_disp_drv_t *drv, lv_area_t *area) {
  ...
  if (x_align > 1) {
    ...
    // round the end of coordinate down to the nearest (x_align * (N + 1) - 1)
    // number
  ...
  if (y_align > 1) {
    ...
    // round the end of coordinate down to the nearest (y_align * (N + 1) - 1)
    // number
...
static lv_disp_t *display_init(ESP_PanelLcd *lcd) {
  ...
  ESP_PANEL_CHECK_FALSE_RET(lcd->getHandle() != nullptr, nullptr,
  ...
  for (int i = 0; (i < LVGL_PORT_BUFFER_NUM) && (i < LVGL_PORT_BUFFER_NUM_MAX);
       i++) {
    buf[i] = heap_caps_malloc(buffer_size * sizeof(lv_color_t),
                              LVGL_PORT_BUFFER_MALLOC_CAPS);
    assert(buf[i]);
    ESP_LOGD(TAG, "Buffer[%d] address: %p, size: %d", i, buf[i],
...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
  ...
  /* Read data from touch controller - thread-safe! */
  if (i2cMutex) {
  ...
  // Always fill data with the last known state.
...
bool lvgl_port_init(ESP_PanelLcd *lcd, ESP_PanelTouch *tp) {
  ...
  ESP_PANEL_CHECK_FALSE_RET(
      lcd->getBus()->getType() == ESP_PANEL_BUS_TYPE_RGB, false,
      "Avoid tearing function only works with RGB LCD now");
  ESP_LOGD(TAG, "Avoid tearing is enabled, mode: %d",
  ...
  ESP_PANEL_CHECK_NULL_RET(disp, false,
  ...
  if (tp != nullptr) {
    ...
    indev = indev_init(tp);
    ESP_PANEL_CHECK_NULL_RET(indev, false,
...
#if LVGL_PORT_AVOID_TEAR
  lcd->attachRefreshFinishCallback(onRgbVsyncCallback,
...
void lcd_init(void) {
  ...
  /**
   * These development boards require the use of an IO expander to configure the
   * screen, so it needs to be initialized in advance and registered with the
   * panel for use.
   *
   */
  Serial.println("Initialize IO expander");
  /* Initialize shared IO expander */
  if (!sdExpander) {
    sdExpander = new ESP_IOExpander_CH422G(
  ...
  delay(100);
  // gt911 initialization, must be added, otherwise the touch screen will not be
  // recognized gt911 初始化，必须要加，否则会无法识别到触摸屏 initialization
  // begin
  ...
  panel->init();
#if LVGL_PORT_AVOID_TEAR
  // When avoid tearing function is enabled, configure the RGB bus according to
  // the LVGL configuration
  ESP_PanelBus_RGB *rgb_bus =
      static_cast<ESP_PanelBus_RGB *>(panel->getLcd()->getBus());
